...
st.session_state.setdefault("s_E", 0.1)
st.session_state.setdefault("s_E_min", 0.08)
st.session_state.setdefault("s_E_max", 0.12)
st.session_state.setdefault("N", 300)
st.session_state.setdefault("detection_threshold", 0.1)
step = 0.01
h = 0.88                       
rr = 1                              
st.session_state.M = 743*st.session_state.N
s = 1
...
def generate_main_plot(tot,f_A, f_a,tot_min,tot_max):
    fig, ax = plt.subplots(figsize=(14, 10), dpi=100)
    ax.plot(np.arange(1, st.session_state.num_generations + 1), np.divide(tot, st.session_state.N), '-r', linewidth=3)
    ax.fill_between(
        np.arange(1, st.session_state.num_generations + 1),
        np.maximum(np.divide(tot_min, st.session_state.N), st.session_state.detection_threshold),
        np.minimum(np.divide(tot_max, st.session_state.N), st.session_state.M / st.session_state.N),
        alpha=0.3,
    )
    print(tot_min)
    print(tot)
...
if main_tab == "Introduction":
    ...
    g_min = h*st.session_state.N*st.session_state.s_E_min*s*(1-st.session_state.bc)
    g_max = h*st.session_state.N*st.session_state.s_E_max*s*(1-st.session_state.bc)
    g_values = [g_min, g, g_max]
    print(g_values)
    ...
    for value in g_values:
        ...
        for n in range(st.session_state.num_generations-1):
            if X[n] + Y[n] + Z[n] == 0:
                ...
                X[n+1] = (value * (M_A[n] * F_A[n] * (X[n] + Y[n]/2)**2) / (M_A[n] * (X[n] + Y[n]) + M_a[n] * Z[n])) / (1 + (X[n] + Y[n] + Z[n]) / st.session_state.M)
                Y[n+1] = (value * (M_A[n] * (F_a[n] * Z[n] + F_A[n] * Y[n]/2) * (X[n] + Y[n]/2) + F_A[n] * (M_a[n] * Z[n] + M_A[n] * Y[n]/2) * (X[n] + Y[n]/2))) / (M_A[n] * (X[n] + Y[n]) + M_a[n] * Z[n]) / (1 + (X[n] + Y[n] + Z[n]) / st.session_state.M)
                Z[n+1] = (value * (F_a[n] * Z[n] + F_A[n] * Y[n]/2) * (M_a[n] * Z[n] + M_A[n] * Y[n]/2)) / (M_A[n] * (X[n] + Y[n]) + M_a[n] * Z[n]) / (1 + (X[n] + Y[n] + Z[n]) / st.session_state.M)
        ...
        for n in range(st.session_state.num_generations):
            if tot_temp[n] == 0:
                ...
                f_AA[n] = X[n] / tot_temp[n]
                f_Aa[n] = Y[n] / tot_temp[n]
                f_aa[n] = Z[n] / tot_temp[n]
    ...
    generate_main_plot(tot,f_A, f_a, tot_min, tot_max) ##Compute this error margin of 0.05 from the error on s_E
    ## Au vu du descriptif de la fonction fill_between, je vais plutôt définir un output tot_min et un output tot_max additionnels. 
    ...
    st.session_state.s_E = st.slider("Survival of larvae from cysts (%):", min_value=st.session_state.s_E_min*100, max_value=st.session_state.s_E_max*100, value=st.session_state.s_E*100, step=0.001)/100
